  ...
  return (
    ...
    >
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <h2 className="text-2xl font-bold text-slate-900">ஆதரவாளர்கள் பட்டியல்</h2>
        <p className="text-sm text-zinc-600">பெயர், மாவட்டம், கையொப்பம், பதிவு தேதி</p>
      </div>

      {loadError ? (
        <p className="rounded-lg border border-slate-300 bg-slate-50 px-3 py-3 text-sm text-slate-700">
          {loadError}
        </p>
      ) : null}
      ...
      {isLoading ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div
              key={`skeleton-${index}`}
              className="loading-pulse rounded-xl border border-slate-200 bg-slate-50 p-4"
            >
      ...
      {!isLoading && supporters.length === 0 ? (
        <p className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-4 text-sm text-zinc-700">
      ...
      {!isLoading && orderedSupporters.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {orderedSupporters.map((supporter, index) => (
            ...
            >
              <p className="text-xs font-semibold text-zinc-500">Serial No: {index + 1}</p>
              <h3 className="text-lg font-bold text-slate-800">{supporter.name}</h3>
              ...
              {supporter.message ? (
                <p className="mt-1 text-xs italic text-zinc-600">"{supporter.message}"</p>
              ) : null}
              <img
